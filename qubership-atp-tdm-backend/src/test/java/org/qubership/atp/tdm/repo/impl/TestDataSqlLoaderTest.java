...
public class TestDataSqlLoaderTest extends AbstractTestDataTest {
    ...
    public void queryTimeout_executeHeavyQuery_throwTimeoutException() throws Exception {
        Exception exception = Assertions.assertThrows(TimeoutException.class, () -> {
            ...
            try(Connection connection = sqlRepository.createConnection(server)) {
                TestDataSqlLoader sqlLoader = new TestDataSqlLoader(connection, QUERY, 1);
                sqlLoader.process();
    ...
    public void sqlRepository_getServer_throwConnectionException() {
        Exception exception = Assertions.assertThrows(TdmEnvDbConnectionException.class, () -> {
            system.getServer("noName");
        });
    }

    @Test
    public void sqlRepository_createConnection_throwTdmDbDrierException() throws Exception {
        Exception exception = Assertions.assertThrows(TdmDbDriverException.class, () -> {
            Server server = new Server(dbConnectionErrorDbType, "DB");
            sqlRepository.createConnection(server);
        });
    }

    @Test
    public void sqlRepository_createConnection_throwTdmDbConnectionException() throws Exception {
        Exception exception = Assertions.assertThrows(TdmDbConnectionException.class, () -> {
